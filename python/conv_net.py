...
def conv_net(params, layers, data, labels, test=False):
    ...
    cost, grad, input_od, percent = mlrloss(
        wb, output[i - 1]["data"], labels, layers[i]["num"], 0, 1
    )
    ...
    if test is False:
        pg = {}
        pg["w"] = (
            np.reshape(grad[: params[i - 1]["w"].size], params[i - 1]["w"].shape)
            / batch_size
        )
        pg["b"] = (
            np.reshape(grad[-params[i - 1]["b"].size :], params[i - 1]["b"].shape)
            / batch_size
        )
        ...
        for i in range(l - 2, 0, -1):
            ...
            if layer_type == "CONV":
                pg, input_od = conv_layer_backward(
                ...
                pg, input_od = inner_product_backward(
                    output[i], output[i - 1], layers[i], params[i - 1]
                )
...
def convnet_forward(params, layers, data, test=False):
    ...
    for i in range(1, l - 1):
        ...
        if layer_type == "CONV":
            ...
            output.append(
                inner_product_forward(output[i - 1], layers[i], params[i - 1])
            )
