...
def load_mnist(fullset=True):
    ...
    ytrain = np.concatenate(
        [i * np.ones((data["train" + str(i)].shape[0], 1)) for i in range(10)]
    )

    xtest = np.vstack([data["test" + str(i)] for i in range(10)])
    ytest = np.concatenate(
        [i * np.ones((data["test" + str(i)].shape[0], 1)) for i in range(10)]
    )
    ...
    if not fullset:
        factor = 20
        xtrain, ytrain = (
            xtrain[:, : xtrain.shape[1] // factor],
            ytrain[:, : ytrain.shape[1] // factor],
        )
        xvalidate, yvalidate = (
            xvalidate[:, : xvalidate.shape[1] // factor],
            yvalidate[:, : yvalidate.shape[1] // factor],
        )
        xtest, ytest = (
            xtest[:, : xtest.shape[1] // factor],
            ytest[:, : ytest.shape[1] // factor],
        )
